...
const CountDownOne = (props: {saleDate: number}) => {
  // Renderer callback with condition
  const renderer = ({ days, hours, minutes, seconds, completed }: RendererType) => {
    if (completed) {
      return (
        <>
  ...
  return (
    <div className="coming-time">
      <Countdown date={props.saleDate} renderer={renderer} />
    </div>
  );
};
